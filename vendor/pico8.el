...
;;; Commentary:
;;; By taylskid.  Also licensed gplv3.  See https://github.com/taylskid/pico8-mode
;;; Code:

(defun pico-find-data1 (limit)
  (let ((start (point)))
    (when (re-search-forward "__lua__" limit t)
      (set-match-data `(,(point-min)
                        ,(match-end 0)))
      (goto-char (match-end 0)))))

(setq pico8-font-lock-keywords (let* ((pico-constants '("true" "false" "nil"))
                                      (pico-keywords '("for" "do" "end" "function" "foreach" "if"
                                                       "then"))
                                      (pico-functions-graphics '("clip" "pget" "pset" "sget" "sset" "fget"
                                                                 "fset" "cursor" "color" "cls" "camera" "circ"
                                                                 "circfill" "line" "rect" "rectfill" "pal"
                                                                 "palt" "spr" "sspr" "fillp"))
                                      (pico-functions-tables '("add" "del" "all" "foreach" "pairs"))
                                      (pico-functions-misc '("btn" "btnp" "sfx" "music" "mget" "mset"
                                                             "map"))
                                      (pico-functions-memory '("peek" "poke" "memcpy" "reload" "cstore"
                                                               "memset"))
                                      (pico-functions-math '("max" "min" "mid" "flr" "cos" "sin" "atan2"
                                                             "sqrt" "abs" "rnd" "srand" "band" "bor" "bxor"
                                                             "bnot" "shl" "shr"))
                                      (pico-events '("print"))
                                      (pico-constants-regexp (regexp-opt pico-constants 'words))
                                      (pico-keywords-regexp (regexp-opt pico-keywords 'words))
                                      (pico-functions-regexp (regexp-opt (append pico-functions-graphics pico-functions-tables
                                                                                 pico-functions-misc pico-functions-memory
                                                                                 pico-functions-math)
                                                                         'words))
